...
public class ImgScroll {
    private final World scrollWorld; // the world the background image scrolls in
    private final GreenfootImage[][] scrollImages; // the drawing panels (background image broken down)
    private final int scrollWidth;
    private final int scrollHeight; // the dimensions of the scrolling area
    private final int worldWidth;
    private final int worldHeight; // the dimensions of the world window
    ...
    public ImgScroll(World scrWorld, GreenfootImage repImage, int wide, int high) {
        ...
        for (int j = 0; j < y; j++)
            for (int i = 0; i < x; i++) // fills the array
            {
